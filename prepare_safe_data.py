...
def process_one_embedding(embedding,
    ...
    if embedding.shape[0] > required_size:
        ...
        embedding = np.stack((embedding, filler))

    assert (embedding.shape[0] == required_size) and (img_embedding.shape[1] == embedding_length)
        ...
        for one_sample in all_samples:
            ...
            if os.path.isfile(target_img_sif_path):
                ...
                if all_img == None:
                    ...
                    all_img = np.stack((all_img, img_embedding))
                ...
                if all_headline == None:
                    all_headline = headline_embedding
                else:
                    all_headline = np.stack((all_headline, headline_embedding))
                ...
                if all_body == None:
                    ...
                    all_body = np.stack((all_body, body_embedding))
                ...
                if all_labels == None:
                    ...
                    all_labels = np.stack((all_labels, one_label))
